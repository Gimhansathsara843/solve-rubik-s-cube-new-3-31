...
public class CubeController {
    ...
    private List<String> getInverseMoves(List<String> scramble) {
        List<String> inverseMoves = new ArrayList<>();
        for (int i = scramble.size() - 1; i >= 0; i--) {
            String move = scramble.get(i);
            if (move.endsWith("'")) {
                inverseMoves.add(move.replace("'", ""));
            } else if (move.endsWith("2")) {
                inverseMoves.add(move); // "2" moves remain the same
            } else {
                inverseMoves.add(move + "'");
            }
        }
        return inverseMoves;
    }
